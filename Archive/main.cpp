...
//    shipsToPlace.printship();
    ...
    BattleShip::board makeBoard (row, col);
//    makeBoard.printboard();
    ...
    if(game_mode == 1){
        player1 = std::make_unique<BattleShip::human_player>(ask_name(1), makeBoard);
        player1->print_setup_board();
        player1->place_ships(shipsToPlace.getShips());
        player2 = std::make_unique<BattleShip::human_player>(ask_name(2), makeBoard);
        ...
        if(ai1_mode == 1){//cheat
            player2 = std::make_unique<BattleShip::cheating_ai>(1, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }else if(ai1_mode == 2){//random
            player2 = std::make_unique<BattleShip::random_ai>(1, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }else{//hunt destroy
            player2 = std::make_unique<BattleShip::hunt_destroy_ai>(1, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }
        ...
        ai1_mode = ask_ai_mode();
        if(ai1_mode == 1){//cheat
            player1 = std::make_unique<BattleShip::cheating_ai>(1, makeBoard);
            player1->place_ships(shipsToPlace.getShips());
        }else if(ai1_mode == 2){//random
            player1 = std::make_unique<BattleShip::random_ai>(1, makeBoard);
            player1->place_ships(shipsToPlace.getShips());
        }else{//hunt destroy
            player1 = std::make_unique<BattleShip::hunt_destroy_ai>(1, makeBoard);
            player1->place_ships(shipsToPlace.getShips());
        }
        ai2_mode = ask_ai_mode();
        if(ai2_mode == 1){//cheat
            player2 = std::make_unique<BattleShip::cheating_ai>(2, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }else if(ai2_mode == 2){//random
            player2 = std::make_unique<BattleShip::random_ai>(2, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }else{//hunt destroy
            player2 = std::make_unique<BattleShip::hunt_destroy_ai>(2, makeBoard);
            player2->place_ships(shipsToPlace.getShips());
        }
    }
